...
actualizarInterfazCombate(estado) {
    // Interfaz para la fase de combate con turnos por tiempo
    this.log(`Actualizando interfaz para fase de combate, subfase: ${estado.subfase}`, null, 'info');
    
    try {
        // Ocultar controles de preparación
        this.ocultarControlesPreparacion();
        
        // Mostrar información de turno
        this.actualizarPanelTurno(estado);
        
        // Mostrar controles de combate
        this.mostrarControlesCombate(estado);
        
        // Configurar timer de turno
        this.configurarTimerTurno(estado);
        
        // Actualizar panel unificado para mostrar controles de turno
        if (window.panelUnificado) {
            window.panelUnificado.mostrar('turnos');
        }
        
    } catch (error) {
        this.log('Error actualizando interfaz de combate:', error, 'error');
        this.mostrarMensaje('Error actualizando interfaz de combate', 'error');
    }
}

actualizarInterfazFinalizacion(estado) {
    // Interfaz para la fase de finalización
    this.log('Actualizando interfaz para fase de finalización', null, 'info');
    
    try {
        // Ocultar todos los controles de juego
        this.ocultarTodosLosControles();
        
        // Mostrar resultados finales
        this.mostrarResultadosFinales(estado);
        
        // Mostrar opciones post-juego
        this.mostrarOpcionesPostJuego();
        
    } catch (error) {
        this.log('Error actualizando interfaz de finalización:', error, 'error');
        this.mostrarMensaje('Error actualizando interfaz de finalización', 'error');
    }
}

// Métodos auxiliares para la interfaz de combate

actualizarPanelTurno(estado) {
    const panelTurno = document.getElementById('estado-juego') || this.crearPanelTurno();
    
    if (panelTurno) {
        panelTurno.innerHTML = `
            <div class="info-combate">
                <div class="turno-info">
                    <strong>Turno ${estado.turnoActual || 1}</strong>
                </div>
                <div class="jugador-info">
                    Jugador: ${estado.jugadorActual?.nombre || 'Desconocido'}
                </div>
                <div class="tiempo-info">
                    <span id="tiempo-restante">${this.formatearTiempo(estado.tiempoRestante || 0)}</span>
                </div>
                <div class="fase-info">
                    ${estado.fase} - ${estado.subfase || 'turno'}
                </div>
            </div>
        `;
    }
...
mostrarControlesCombate(estado) {
    // Crear o actualizar controles específicos de combate
    const controlesCombate = document.getElementById('controles-combate') || this.crearControlesCombate();
    
    if (controlesCombate) {
        controlesCombate.innerHTML = `
            <div class="acciones-combate">
                <button id="btn-mover-unidades" class="btn-combate">
                    <i class="fas fa-arrows-alt"></i> Mover Unidades
                </button>
                <button id="btn-atacar" class="btn-combate">
                    <i class="fas fa-crosshairs"></i> Atacar
                </button>
                <button id="btn-defender" class="btn-combate">
                    <i class="fas fa-shield-alt"></i> Defender
                </button>
                <button id="btn-finalizar-turno" class="btn-combate peligro" onclick="panelUnificado.finalizarTurno()">
...
configurarTimerTurno(estado) {
    // Actualizar el timer visual cada segundo
    ...
    this.intervalTimer = setInterval(() => {
        ...
        if (tiempoElement && estado.tiempoRestante > 0) {
            estado.tiempoRestante--;
            tiempoElement.textContent = this.formatearTiempo(estado.tiempoRestante);
            
            // Cambiar color cuando queda poco tiempo
            if (estado.tiempoRestante <= 30) {
                tiempoElement.classList.add('tiempo-critico');
            } else if (estado.tiempoRestante <= 60) {
                tiempoElement.classList.add('tiempo-advertencia');
            }
        }
    }, 1000);
}
...
crearPanelTurno() {
    ...
    panel.id = 'estado-juego';
    panel.className = 'panel-turno';
    document.body.appendChild(panel);
    return panel;
}

crearControlesCombate() {
    const controles = document.createElement('div');
    controles.id = 'controles-combate';
    controles.className = 'controles-combate';
...
mostrarResultadosFinales(estado) {
    ...
    resultados.className = 'panel-resultados';
    resultados.innerHTML = `
        <h2>🎯 Fin del Juego</h2>
        <div class="resultados-contenido">
            <p>La partida ha finalizado</p>
            <div class="estadisticas">
                <div>Duración total: ${this.calcularDuracionPartida()}</div>
                <div>Turnos completados: ${estado.turnoActual || 0}</div>
            </div>
        </div>
    `;
...
mostrarOpcionesPostJuego() {
    // Opciones después del juego
    const opciones = document.createElement('div');
    opciones.id = 'opciones-post-juego';
    opciones.className = 'panel-opciones';
    opciones.innerHTML = `
        <div class="opciones-contenido">
            <button onclick="window.location.href='iniciarpartida.html'" class="btn-opcion">
                <i class="fas fa-redo"></i> Nueva Partida
            </button>
            <button onclick="window.location.href='planeamiento.html'" class="btn-opcion">
                <i class="fas fa-map"></i> Ir a Planeamiento
            </button>
            <button onclick="this.exportarResultados()" class="btn-opcion">
...
calcularDuracionPartida() {
    // Calcular duración desde el inicio del juego
    if (this.inicioPartida) {
        const duracion = Math.floor((Date.now() - this.inicioPartida) / 1000);
        return this.formatearTiempo(duracion);
    }
    return "00:00";
}
