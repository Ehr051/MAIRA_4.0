...
class MiniTilesLoader {
    constructor() {
        ...
        this.baseUrls = [
            'https://github.com/Ehr051/MAIRA_4.0/releases/download/v4.0/',        // GitHub Release v4.0 - PRINCIPAL
            '../../mini_tiles_github/',                                           // Local dev con estructura (para testing)
            'https://cdn.jsdelivr.net/gh/Ehr051/MAIRA_4.0@main/mini_tiles_github/'   // CDN (backup, con estructura)
        ];
    ...
    async getTile(lat, lon, tipo = 'altimetria') {
        ...
        if (tipo === 'vegetation' || tipo === 'vegetacion') {
            return this.getVegetationTile(lat, lon);
        }
        
        // Encontrar provincia para estas coordenadas (altimetría)
    ...
    /**
     * Obtiene tile de vegetación para coordenadas específicas
     */
    async getVegetationTile(lat, lon) {
        // URLs específicas para vegetación (v4.0)
        const vegetationUrls = [
            ...
            'https://cdn.jsdelivr.net/gh/Ehr051/MAIRA_4.0@main/Client/Libs/datos_argentina/Vegetacion_Mini_Tiles/'
        ];
        
        // Determinar batch basado en coordenadas
        const batch = this.getBatchForCoordinates(lat, lon);
        const tileFilename = `vegetation_ndvi_${Math.floor(lat*1000)}_${Math.floor(lon*1000)}.tif`;
        
        console.log(`🌿 Cargando tile de vegetación: ${tileFilename} (batch: ${batch})`);
        
        // Intentar cargar desde diferentes URLs
        for (const baseUrl of vegetationUrls) {
            try {
                const url = `${baseUrl}${batch}/${tileFilename}`;
                const response = await fetch(url);
                
                if (response.ok) {
                    const tileData = await response.arrayBuffer();
                    console.log(`✅ Tile de vegetación cargado desde: ${baseUrl}`);
                    return {
                        data: tileData,
                        type: 'vegetation',
                        batch: batch,
                ...
                console.warn(`⚠️ Error cargando vegetación desde ${baseUrl}:`, error);
            }
        }
        
        console.warn(`❌ No se pudo cargar tile de vegetación para ${lat}, ${lon}`);
        return null;
    }

    /**
     * Determina el batch de vegetación para coordenadas
     */
    getBatchForCoordinates(lat, lon) {
        // Algoritmo simple para distribuir en 17 batches
        const latIndex = Math.floor((lat + 90) / 10);
        const lonIndex = Math.floor((lon + 180) / 20);
        const batchNumber = (latIndex * 18 + lonIndex) % 17 + 1;
        
        return `vegetation_ndvi_batch_${batchNumber.toString().padStart(2, '0')}`;
    }
...
/**
 * TileLoaderService - Wrapper para compatibilidad con el sistema de servicios
 */
class TileLoaderService {
    constructor(core) {
        this.core = core;
        this.miniTilesLoader = new MiniTilesLoader();
    }

    async loadTile(type, tileId) {
        // Convertir tileId a coordenadas (implementación simplificada)
        const [lat, lon] = this.tileIdToCoordinates(tileId);
        
        if (type === 'elevation') {
            return this.miniTilesLoader.getTile(lat, lon, 'altimetria');
        } else if (type === 'vegetation') {
            return this.miniTilesLoader.getTile(lat, lon, 'vegetacion');
        }
        
        throw new Error(`Tipo de tile no soportado: ${type}`);
    }

    tileIdToCoordinates(tileId) {
        // Implementación básica - ajustar según el formato real de tileId
        const parts = tileId.split('_');
        const lat = parseFloat(parts[0]) || -34.6037;  // Buenos Aires por defecto
        const lon = parseFloat(parts[1]) || -58.3816;
        return [lat, lon];
    }

    getCacheStats() {
        return this.miniTilesLoader.getStats();
    }
}
...
console.log('🚀 Mini-tiles Loader v4.0 disponible globalmente');

// Export para módulos ES6
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { MiniTilesLoader, TileLoaderService };
}

// Export para ES6 imports
export { MiniTilesLoader, TileLoaderService };
